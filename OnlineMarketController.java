...
public class OnlineMarketController extends UnicastRemoteObject implements OnlineMarket{
	...
	public synchronized String addItemsToCart(Session session,int itemId,int itemQuantity){
		OnlineMarketModel modelObj= new OnlineMarketModel();
		return modelObj.addItemsToCart(itemId,itemQuantity);
	}
	...
	@Override
	public synchronized String addItems(Session session,int itemId,String itemName,String itemType,String itemPrice, int itemQuant){
		OnlineMarketModel modelObj= new OnlineMarketModel();
		return modelObj.addItems(itemId,itemName,itemType,itemPrice,itemQuant);
	}

	//overridden sync method to add items
	@Override
	public synchronized String updateItems(Session session,int itemId,String itemAttribute){
		OnlineMarketModel modelObj= new OnlineMarketModel();
		return modelObj.updateItems(itemId,itemName,itemAttribute);
	}
